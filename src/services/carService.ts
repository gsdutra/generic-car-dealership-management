...
async function updateCar(model: string, licensePlate: string, year: number, color: string) {
  const up = await carRepository.carUpdate(model,licensePlate,year,color)
  if(up) throw ("Car Updated")
}
